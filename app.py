...
def init_session_state():
    for k, v in {"user": None, "page": "home", "detected_lat": None, "detected_lng": None, "detected_address": None}.items():
...
<div class="main-header">
    <h1>🍽️ Community Surplus Food Sharing</h1>
...
def donor_page():
    ...
    # Always render the geolocation widget
    ...
    with st.form("new_listing"):
        ...
        # Only enable submit if location is detected
            ...
            if st.button("TAKEAWAY", key=f"claim_{item['id']}"):
                ...
                if claim_id:
                    ...
                    try:
                        ...
                        notification_message = f"{receiver.get('name', 'Someone')} wants to take your food: {item.get('title', '')}"
                        ...
                        if notification_id:
                            ...
                            print("🔄 Attempting to recreate notifications table...")
                            from db import recreate_notifications_table
                            if recreate_notifications_table():
                                st.info("🔄 Notifications table recreated. Please try claiming again.")
                            else:
                                st.error("⚠ Failed to fix notifications system")
